...
2. Run any local validation the repo supports before pushing:
   - Check for a `Makefile` and run the default target or test target if available.
   - Check for `package.json` scripts (`npm test`, `npm run lint`).
   - If no validation tooling is found, skip this step.
...
6. Write/update PR body:
   - If `.github/pull_request_template.md` exists, use it as the body template
     and fill every section with concrete content for this change.
   - If no PR template exists, write a clear body with: summary of changes,
     motivation, testing done, and any notes for reviewers.
